...
## UPDATE 54 — RLS Step 2: ObjectGraphRepository READS (IMPLEMENTED, reads-only split — Slice 40a)

Tenant-scopes the **3 self-contained read methods** of `ObjectGraphRepository` (the central canonical
`objects` table + `external_references`). This is the FIRST **live-wired central-store** adopter — the
repo is wired in the production API (`apps/api/src/shared/container.ts:56-57`) and read by GraphQL
resolvers, the consent engine, retrieval, identity-resolution, and the intake orchestrator — so the
audit split it: **reads now (low-risk, GUC-inert), writes deferred** to a coordinated slice. **No
functional change** — proven by the existing `object-graph.test.ts` (which drives `create`/`update` →
internal `getById`) and the wider API/engine suites still passing.

- `packages/core/src/object-graph/repository.ts` — each read now
  `withTenantTransaction(this.db, tenantId, client => …)`:
  - `getById` — `SELECT id, tenant_id, type, state, attributes, version, created_at, updated_at FROM
    objects WHERE id=$1 AND tenant_id=$2` (moved `this.db.queryOne` → `(await client.query()).rows[0]
    ?? null`, same SQL/params/mapping).
  - `getExternalReferences` — `SELECT system, ext_type, value FROM external_references WHERE
    object_id=$1 AND tenant_id=$2`.
  - `findByExternalReference` — the `objects ⋈ external_references` JOIN, byte-identical.
- **Untouched (out of scope, deferred to 40b):** `create`, `update`, `addExternalReference`,
  `createWithClient` (incl. its by-id readback `SELECT * FROM objects WHERE id=$1`), the
  `ObjectCommandHandler` transactions, and EventStore. `getById` is still called internally by
  `create`/`update`, but those run their INSERT/UPDATE on `this.db` and then call `getById`
  **sequentially** (they do not open a transaction around it) → the now-wrapped `getById` opens its own
  top-level transaction; **no nesting**.
- **Preserved exactly:** SQL text (incl. the multi-line JOIN), params, row mapping, `null`/array
  returns, public signatures. GUC only — NO policy/`FORCE`/`WITH CHECK`. ⚠ `findByExternalReference`'s
  JOIN does not filter `er.tenant_id` — constrained via `er.object_id=o.id` + `o.tenant_id=$1`, so it
  cannot return foreign-tenant objects; flag for the RLS-enablement slice.
- **Why writes are deferred (the by-id special case + shared-`objects` coordination):** the production
  write path is the `ObjectCommandHandler` `this.db.transaction(client => …)` which passes the client to
  `createWithClient` (its by-id readback has no tenant predicate) and `EventStore.append` (same client,
  same transaction). RLS on the write path requires the GUC set on the **handler's** transaction — a
  coordinated change with EventStore, and the prerequisite for ENABLING RLS on `objects`/`events`. To
  be audited jointly with EventStore (Slice 40b).
- Unit (`object-graph-tenant.test.ts`, new, 6 tests): mocked `DatabaseClient` proving each read — one
  transaction, GUC once-and-first, byte-identical (normalized) SQL + params, correct mapping, and
  `null`/empty-array returns. Postgres-free.
- Harness (`tenant-scope.integration.test.ts`, +1): real-Postgres on migration-001-faithful fixtures
  (`objects.attributes` JSONB + `created_at`/`updated_at`; `external_references` PK
  `(object_id, system, ext_type)`) — `getById`/`getExternalReferences`/`findByExternalReference` return
  tenant-local rows and **empty/null for the wrong tenant**, including the JOIN returning only the
  caller-tenant object when the same `(system, ext_type, value)` exists for both tenants. Self-skipping.
- Default verify green (core 716 pass/18 skip; API 218 pass; build:all clean).
